...
class RandomSubsetSampler(Sampler):
    ...
    def __iter__(self):
        return iter(np.random.choice(len(self.data_source), self.sample_size, replace=False))
...
sample_size_val = 40000
# 获取文件路径
# 多目标改变信噪比
# file_path = r"C:\Users\buaa\Desktop\毕设相关\孔径扩展\matlab部分\TrainData_oriSig_2Target_changeSNR\Train_oriSig_Array_10_20_target_2_-10dB.mat"
# 多目标改变信噪比
file_path_train = r"C:\Users\buaa\Desktop\毕设相关\孔径扩展\matlab部分\TrainData_oriSig_2Target_changeSNR\Train_oriSig_Array_10_20_theta_-60_52_target_2_10dB.mat"
file_path_val = r"C:\Users\buaa\Desktop\毕设相关\孔径扩展\matlab部分\ValData_oriSig_2Target_changeSNR\Val_oriSig_Array_10_20_theta_-60_52_target_2_10dB.mat"
# # 多目标改变角度
...
# 将 (16000, 10, SNAP) 的数据展平为 (16000, 10*SNAP)
# # 训练集数据转换
# train_L_R_flat = train_L_R.transpose((0, 2, 1)).reshape(-1, train_L_R.shape[1])
# train_L_I_flat = train_L_I.transpose((0, 2, 1)).reshape(-1, train_L_I.shape[1])
# train_L = np.concatenate((train_L_R_flat, train_L_I_flat), axis=1)
#
#
# train_H_R_flat = train_H_R.transpose((0, 2, 1)).reshape(-1, train_H_R.shape[1])
# train_H_I_flat = train_H_I.transpose((0, 2, 1)).reshape(-1, train_H_I.shape[1])
# train_H = np.concatenate((train_H_R_flat, train_H_I_flat), axis=1)
#
# # 测试集数据转换
# val_L_R_flat = val_L_R.transpose((0, 2, 1)).reshape(-1, val_L_R.shape[1])
# val_L_I_flat = val_L_I.transpose((0, 2, 1)).reshape(-1, val_L_I.shape[1])
# val_L = np.concatenate((val_L_R_flat, val_L_I_flat), axis=1)
#
#
# val_H_R_flat = val_H_R.transpose((0, 2, 1)).reshape(-1, val_H_R.shape[1])
# val_H_I_flat = val_H_I.transpose((0, 2, 1)).reshape(-1, val_H_I.shape[1])
# val_H = np.concatenate((val_H_R_flat, val_H_I_flat), axis=1)

# train_L = np.concatenate((train_L_R, train_L_I), axis=1)
train_H = np.concatenate((train_H_R, train_H_I), axis=1)
# val_L = np.concatenate((val_L_R, val_L_I), axis=1)
val_H = np.concatenate((val_H_R, val_H_I), axis=1)
train_L = np.concatenate((train_L_R, train_L_I), axis=1)
# train_H = np.stack((train_H_R, train_H_I), axis=2)
val_L = np.concatenate((val_L_R, val_L_I), axis=1)
# val_H = np.stack((val_H_R, val_H_I), axis=2)
...
# 将形状 (B, 2, H) 转换为 (B, 2*H)
H_L = train_L_R.shape[1]
H_H = train_H_R.shape[1]
train_L = train_L.reshape(train_L.shape[0], -1)
# train_H_reshaped = train_H.reshape(train_H.shape[0], -1)
val_L = val_L.reshape(val_L.shape[0], -1)
# val_H_reshaped = val_H.reshape(val_H.shape[0], -1)
# Normalization，分别对两个尺寸的阵列进行标准化，用训练集进行fit，测试集只进行transform
train_L = mm_scalerRT_INPUT_L.fit_transform(train_L)

val_L = mm_scalerRT_INPUT_L.transform(val_L)

# Normalization
train_H = mm_scalerRT_INPUT_H.fit_transform(train_H)

val_H = mm_scalerRT_INPUT_H.transform(val_H)

train_L = train_L.reshape(train_L.shape[0], H_L*2)
# train_H = train_H.reshape(train_H.shape[0], 2, H_H)
val_L = val_L.reshape(val_L.shape[0], H_L* 2)
# val_H = val_H.reshape(val_H.shape[0], 2, H_H)

# 创建每次训练的文件夹。保存归一化工具和训练模型
dirStore = f"./" + file_name + "_DNN_" + nowTime
...
## 保存归一化工具的参数
# filename = dirStore + f"/Train_Array_scalarL_Theta_{Theta0}_{Theta1}_{SNR}dB_DNN"
filename = dirStore + f"/Train_Array_scalarL_DNN"
joblib.dump(mm_scalerRT_INPUT_L, filename)
filename = dirStore + f"/Train_Array_scalarH_DNN"
joblib.dump(mm_scalerRT_INPUT_H, filename)


# # 将数据reshape成一个二维数组，因为归一化对矩阵作用的话，是对每一列进行的
# train_L = train_L.reshape(-1, 2, 10, 10)
# train_H = train_H.reshape(-1, 2, 20, 20)
# val_L = val_L.reshape(-1, 2, 10, 10)
# val_H = val_H.reshape(-1, 2, 20, 20)
...
# 使用随机采样，数据太多了
# train_loader = DataLoader(train_dataset, batch_size=batch_size, sampler=RandomSubsetSampler(train_dataset, sample_size_train))
val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
# val_loader = DataLoader(val_dataset, batch_size=batch_size, sampler=RandomSubsetSampler(val_dataset, sample_size_val))

# 检查是否有可用的 GPU
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# 模型参数
# input_dim = train_L.shape[1]
# output_dim = train_H.shape[1]
input_dim = train_L.shape[1]
output_dim = train_H.shape[1]
# 实例化模型并将其移动到 GPU
model = OriSigDNN(20, 40).to(device)
# 打印模型结构
print(model)
...
# # 创建一个随机输入张量
# input_tensor = torch.randn(1,train_L.shape[1]).to(device)  # 假设输入图片大小为 32x32，通道数为 3
# # 通过模型前向传播计算输出
...
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.2)  # 每10个epoch学习率减少为原来的0.1倍
...
# model.cpu()
# # 保存模型的参数
# filename = dirStore + f"/"+file_name+"_DNN"
